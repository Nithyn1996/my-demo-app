...
public class PortionResource extends APIFixedConstant {
	...
	@ApiOperation(value = "View Portion Details",
				  nickname = "PortionDetailView",
				  notes = "View Portion Details")
	@ApiResponses(value = {
	...
	@RequestMapping(value = "/portion", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<?> viewPortionDetail(HttpServletRequest httpRequest,
			...
			@RequestParam(name = "type", defaultValue = "PORTION", required = true) String type,
		...
		String apiLogString     = "LOG_API_PORTION_VIEW";
    	String apiInstLogString = "LOG_INST_PORTION_VIEW";

		sortOrder = (sortOrder.equals("")) ? GC_SORT_ASC : sortOrder;
    	sortBy    = (sortBy.equals("")) ? GC_SORT_BY_CREATED_BY : sortBy;
    	type	  = (type.length() <= 0) ? APIResourceName.PORTION.toString() : type;
    	String resourceType 	= APIResourceName.PORTION.toString();
		...
		if (!FieldValidator.isValidInternalTextOptional(type))
  		...
  		if (errorList.size() == 0) {
			...
			if (roleStatucCode.equals(GC_STATUS_SUCCESS)) {
				...
				List<Portion> succPayload = portionService.viewListByCriteria(companyId, groupId, divisionId, moduleId, userId, propertyId, sectionId, id, category, statusList, typeList, sortBy, sortOrder, offset, limit);
	...
	@ApiOperation(value = "Create Portion Details",
				  nickname = "PortionDetailCreate",
				  notes = "Create Portion Details")
	@ApiResponses(value = {
	...
	@RequestMapping(value = "/portion", method = RequestMethod.POST, consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
 	public ResponseEntity<?> createPortionDetail(HttpServletRequest httpRequest,
			@RequestHeader(value = "referredBy", defaultValue = "", required = true) String referredBy,
			@RequestHeader(value = "sessionId", defaultValue = "", required = true) String sessionId,
			@RequestBody Portion portionReq) {

		Timestamp dbOperationStartTime = APIDateTime.getGlobalDateTimeDBOperation();
		Timestamp instrumentStartTime  = APIDateTime.getGlobalDateTimeLogOperation();

		String apiLogString     = "LOG_API_PORTION_CREATE";
    	String apiInstLogString = "LOG_INST_PORTION_CREATE";
		...
		Set<ConstraintViolation<Portion>> validationErrors = validator.validate(portionReq);

    	List<APIPreConditionErrorField> errorList = FieldValidator.isValidHeaderFieldValue(referredBy, sessionId);
		errorList.addAll(payloadValidator.isValidPortionPayload(GC_METHOD_POST, portionReq));
    	for (ConstraintViolation<Portion> error : validationErrors)
  		...
  		if (errorList.size() == 0) {
			...
			if (roleStatucCode.equals(GC_STATUS_SUCCESS)) {
				...
				ProfileDependencyAccess profileDependency = profileDependencyService.portionProfileDependencyStatus(GC_METHOD_POST, portionReq);
				...
				if (profileDependencyCode.equals(GC_STATUS_SUCCESS)) {
					...
					ProfileCardinalityAccess profileCardinality = profileCardinalityService.portionProfileCardinalityStatus(GC_METHOD_POST, portionReq);
					...
					if (profileCardinalityCode.equals(GC_STATUS_SUCCESS)) {
						...
						portionReq.setCreatedAt(dbOperationStartTime);
						portionReq.setModifiedAt(dbOperationStartTime);
						portionReq.setCreatedBy(sessionUserId);
						portionReq.setModifiedBy(sessionUserId);

						Portion succPayload = portionService.createPortion(portionReq);
