...
public class UserAuthenticationResource extends APIFixedConstant {
	...
	@ApiOperation(value = "View User Authentication Details",
				  nickname = "UserAuthenticationDetailView",
				  notes = "View User Authentication Details")
	@ApiResponses(value = {
        @ApiResponse(code = SC_STATUS_OK, message = SM_STATUS_OK, response = Portion.class, responseContainer = "List"),
	...
	@RequestMapping(value = "/userAuthentication", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<?> viewUserAuthenticationDetail(HttpServletRequest httpRequest,
			...
			@RequestParam(name = "deviceUniqueId", defaultValue = "", required = false) String deviceUniqueId,
			@RequestParam(name = "category", defaultValue = "", required = false) String category,
			@RequestParam(name = "status", defaultValue = "", required = false) String status,
			@RequestParam(name = "type", defaultValue = "USER_AUTHENTICATION", required = true) String type,
		...
		String apiLogString     = "LOG_API_USER_AUTHENTICATION_VIEW";
    	String apiInstLogString = "LOG_INST_USER_AUTHENTICATION_VIEW";

		sortOrder = (sortOrder.equals("")) ? GC_SORT_ASC : sortOrder;
    	sortBy    = (sortBy.equals("")) ? GC_SORT_BY_CREATED_BY : sortBy;
    	type	  = (type.length() <= 0) ? APIResourceName.USER_AUTHENTICATION.toString() : type;
		...
		if (!FieldValidator.isValidInternalTextOptional(type))
  		...
  		if (errorList.size() == 0) {
			...
			if (roleStatucCode.equals(GC_STATUS_SUCCESS)) {
				...
				List<UserAuthentication> succPayload = userAuthenticationService.viewListByCriteria(companyId, divisionId, moduleId, userId, id, deviceUniqueId, category, statusList, typeList, sortBy, sortOrder, offset, limit);
	...
	@ApiOperation(value = "Create User Authentication Details",
				  nickname = "UserAuthenticationDetailCreate",
				  notes = "Create User Authentication Details")
	@ApiResponses(value = {
        @ApiResponse(code = SC_STATUS_CREATED, message = SM_STATUS_CREATED, response = Portion.class),
        @ApiResponse(code = SC_STATUS_FORBIDDEN, message = SM_STATUS_FORBIDDEN, response = APIRunTimeError.class),
        @ApiResponse(code = SC_STATUS_PRE_CONDITION, message = SM_STATUS_PRE_CONDITION, response = APIPreConditionError.class),
        @ApiResponse(code = SC_STATUS_FAILED_DEPENDENCY, message = SM_STATUS_FAILED_DEPENDENCY, response = APIRunTimeError.class),
        @ApiResponse(code = SC_STATUS_INTERNAL, message = SM_STATUS_INTERNAL, response = APIRunTimeError.class)
      })
	@RequestMapping(value = "/userAuthentication", method = RequestMethod.POST, consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
 	public ResponseEntity<?> createUserAuthenticationDetail(HttpServletRequest httpRequest,
			@RequestHeader(value = "referredBy", defaultValue = "", required = true) String referredBy,
			@RequestHeader(value = "sessionId", defaultValue = "", required = true) String sessionId,
			@RequestBody UserAuthentication userAuthenticationReq) {

		Timestamp dbOperationStartTime = APIDateTime.getGlobalDateTimeDBOperation();
		Timestamp instrumentStartTime  = APIDateTime.getGlobalDateTimeLogOperation();

		String apiLogString     = "LOG_API_USER_AUTHENTICATION_CREATE";
    	String apiInstLogString = "LOG_INST_USER_AUTHENTICATION_CREATE";
		...
		Set<ConstraintViolation<UserAuthentication>> validationErrors = validator.validate(userAuthenticationReq);

    	List<APIPreConditionErrorField> errorList = FieldValidator.isValidHeaderFieldValue(referredBy, sessionId);
		errorList.addAll(payloadValidator.isValidUserAuthenticationPayload(GC_METHOD_POST, userAuthenticationReq));
    	for (ConstraintViolation<UserAuthentication> error : validationErrors)
  		...
  		if (errorList.size() == 0) {
			...
			if (roleStatucCode.equals(GC_STATUS_SUCCESS)) {
				...
				ProfileDependencyAccess profileDependency = profileDependencyService.userAuthenticationProfileDependencyStatus(GC_METHOD_POST, userAuthenticationReq);
				...
				if (profileDependencyCode.equals(GC_STATUS_SUCCESS)) {
					...
					ProfileCardinalityAccess profileCardinality = profileCardinalityService.userAuthenticationProfileCardinalityStatus(GC_METHOD_POST, userAuthenticationReq);
					...
					if (profileCardinalityCode.equals(GC_STATUS_SUCCESS)) {
						...
						userAuthenticationReq.setCreatedAt(dbOperationStartTime);
						userAuthenticationReq.setModifiedAt(dbOperationStartTime);
						userAuthenticationReq.setCreatedBy(sessionUserId);
						userAuthenticationReq.setModifiedBy(sessionUserId);

						UserAuthentication succPayload = userAuthenticationService.createUserAuthentication(userAuthenticationReq);
	...
	@ApiOperation(value = "Remove User Authentication Details",
			  nickname = "UserAuthenticationRemove",
			  notes = "Remove User Authentication Details")
	@ApiResponses(value = {
	...
	@RequestMapping(value = "/userAuthentication", method = RequestMethod.DELETE, produces = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<?> removeUserAuthenticationDetail(HttpServletRequest httpRequest,
			@RequestHeader(value = "referredBy", defaultValue = "", required = true) String referredBy,
			@RequestHeader(value = "sessionId", defaultValue = "", required = true) String sessionId,
			@RequestParam(name = "companyId", defaultValue = "", required = true) String companyId,
			@RequestParam(name = "divisionId", defaultValue = "", required = false) String divisionId,
		...
		Timestamp instrumentStartTime = APIDateTime.getGlobalDateTimeLogOperation();
		String apiLogString     = "LOG_API_USER_AUTHENTICATION_REMOVE";
		String apiInstLogString = "LOG_INST_USER_AUTHENTICATION_REMOVE";
    	String resourceType = APIResourceName.DEVICE.toString();
		...
		if (errorList.size() == 0) {
			...
			List<UserAuthentication> usrAuthenList = userAuthenticationService.viewListByCriteria(companyId, divisionId, moduleId, userId, id, "", "", emptyList, emptyList, "", "", 0, 0);

	  		if (usrAuthenList.size() > 0) {

	  			String type = usrAuthenList.get(0).getType();
				...
				if (roleStatucCode.equals(GC_STATUS_SUCCESS)) {
					...
					int results = userAuthenticationService.removeDeviceById(companyId, userId, id);
