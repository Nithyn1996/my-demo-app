...
public class SectionResource extends APIFixedConstant {
	...
	@Autowired
	ProfileDependencyService profileDependencyAccessService;
	@Autowired
	ProfileCardinalityService profileCardinalityAccessService;
	...
	@ApiOperation(value = "View Section Details",
				  nickname = "SectionDetailView",
				  notes = "View Section Details")
	@ApiResponses(value = {
        @ApiResponse(code = SC_STATUS_OK, message = SM_STATUS_OK, response = Section.class, responseContainer = "List"),
	...
	@RequestMapping(value = "/section", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<?> viewSectionDetail(HttpServletRequest httpRequest,
			...
			@RequestParam(name = "type", defaultValue = "SECTION", required = true) String type,
		...
		String apiLogString     = "LOG_API_SECTION_VIEW";
    	String apiInstLogString = "LOG_INST_SECTION_VIEW";

		sortOrder = (sortOrder.equals("")) ? GC_SORT_ASC : sortOrder;
    	sortBy    = (sortBy.equals("")) ? GC_SORT_BY_CREATED_BY : sortBy;
    	type	  = (type.length() <= 0) ? APIResourceName.SECTION.toString() : type;
    	String resourceType 	= APIResourceName.SECTION.toString();
		...
		if (!FieldValidator.isValidInternalTextOptional(type))
  		...
  		if (errorList.size() == 0) {
			...
			if (roleStatucCode.equals(GC_STATUS_SUCCESS)) {
				...
				List<Section> succPayload = sectionService.viewListByCriteria(companyId, groupId, divisionId, moduleId, userId, propertyId, id, category, statusList, typeList, sortBy, sortOrder, offset, limit);
	...
	@ApiOperation(value = "Create Section Details",
				  nickname = "SectionDetailCreate",
				  notes = "Create Section Details")
	@ApiResponses(value = {
        @ApiResponse(code = SC_STATUS_CREATED, message = SM_STATUS_CREATED, response = Section.class),
        @ApiResponse(code = SC_STATUS_FORBIDDEN, message = SM_STATUS_FORBIDDEN, response = APIRunTimeError.class),
        @ApiResponse(code = SC_STATUS_PRE_CONDITION, message = SM_STATUS_PRE_CONDITION, response = APIPreConditionError.class),
        @ApiResponse(code = SC_STATUS_FAILED_DEPENDENCY, message = SM_STATUS_FAILED_DEPENDENCY, response = APIRunTimeError.class),
        @ApiResponse(code = SC_STATUS_INTERNAL, message = SM_STATUS_INTERNAL, response = APIRunTimeError.class)
      })
	@RequestMapping(value = "/section", method = RequestMethod.POST, consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
 	public ResponseEntity<?> createSectionDetail(HttpServletRequest httpRequest,
			@RequestHeader(value = "referredBy", defaultValue = "", required = true) String referredBy,
			@RequestHeader(value = "sessionId", defaultValue = "", required = true) String sessionId,
			@RequestBody Section sectionReq) {

		Timestamp dbOperationStartTime = APIDateTime.getGlobalDateTimeDBOperation();
		Timestamp instrumentStartTime  = APIDateTime.getGlobalDateTimeLogOperation();

		String apiLogString     = "LOG_API_SECTION_CREATE";
    	String apiInstLogString = "LOG_INST_SECTION_CREATE";
		...
		Set<ConstraintViolation<Section>> validationErrors = validator.validate(sectionReq);

    	List<APIPreConditionErrorField> errorList = FieldValidator.isValidHeaderFieldValue(referredBy, sessionId);
		errorList.addAll(payloadValidator.isValidSectionPayload(GC_METHOD_POST, sectionReq));
    	for (ConstraintViolation<Section> error : validationErrors)
  		...
  		if (errorList.size() == 0) {
			...
			if (roleStatucCode.equals(GC_STATUS_SUCCESS)) {
				...
				ProfileDependencyAccess profileDependency = profileDependencyAccessService.sectionProfileDependencyStatus(GC_METHOD_POST, sectionReq);
				...
				if (profileDependencyCode.equals(GC_STATUS_SUCCESS)) {
					...
					ProfileCardinalityAccess profileCardinality = profileCardinalityAccessService.sectionProfileCardinalityStatus(GC_METHOD_POST, sectionReq);
					...
					if (profileCardinalityCode.equals(GC_STATUS_SUCCESS)) {
						...
						sectionReq.setCreatedAt(dbOperationStartTime);
						sectionReq.setModifiedAt(dbOperationStartTime);
						sectionReq.setCreatedBy(sessionUserId);
						sectionReq.setModifiedBy(sessionUserId);

						Section succPayload = sectionService.createSection(sectionReq);
