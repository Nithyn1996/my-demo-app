...
import com.common.api.resultset.QueryMapResultset;
import com.common.api.util.ResultSetConversion;

@Service
public class MSQueryMapService extends APIFixedConstant implements QueryMapService {
	...
	public List<QueryMap> viewListByCriteria(String companyId, String groupId, String divisionId, String moduleId, String id, String procedureName, String queryType, String sortBy, String sortOrder, int offset, int limit) {
		...
		if (queryType != null && queryType.length() > 0) {
			selQuery = selQuery + " AND LOWER(query_type) = LOWER(:queryType) ";
			parameters.addValue("queryType", queryType);
		}
		...
		if (selQuery.length() > 0) {
			...
			selQuery = "select * from " + TB_QUERY_MAP + " where " + selQuery;
			return namedParameterJdbcTemplate.query(selQuery, parameters, new QueryMapResultset());
		}
