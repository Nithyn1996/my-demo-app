...
import com.common.api.resultset.DashboardMapResultset;
import com.common.api.util.ResultSetConversion;

@Service
public class MSDashboardMapService extends APIFixedConstant implements DashboardMapService {
	...
	public List<DashboardMap> viewListByCriteria(String companyId, String groupId, String divisionId, String moduleId, String id, String procedureName, String dashboardType, String sortBy, String sortOrder, int offset, int limit) {
		...
		if (dashboardType != null && dashboardType.length() > 0) {
			selQuery = selQuery + " AND LOWER(dashboard_type) = LOWER(:dashboardType) ";
			parameters.addValue("dashboardType", dashboardType);
		}
		...
		if (selQuery.length() > 0) {
			...
			selQuery = "select * from " + TB_DASHBOARD_MAP + " where " + selQuery;
			return namedParameterJdbcTemplate.query(selQuery, parameters, new DashboardMapResultset());
		}
